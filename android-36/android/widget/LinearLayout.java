...
public class LinearLayout extends ViewGroup {
    ...
    void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
        // 重置总长度为0，将累加所有子View的宽度
        mTotalLength = 0;
        // 最大高度，用于确定LinearLayout的最终高度
        int maxHeight = 0;
        // 子View的状态，合并所有子View的测量状态
        int childState = 0;
        // 备选最大高度，用于非权重子View
        int alternativeMaxHeight = 0;
        // 权重子View的最大高度
        int weightedMaxHeight = 0;
        // 是否所有子View都是MATCH_PARENT高度
        boolean allFillParent = true;
        // 所有子View的权重总和
        float totalWeight = 0;

        // 获取虚拟子View数量（LinearLayout中就是实际子View数量）
        final int count = getVirtualChildCount();
        ...
        // 是否有子View需要MATCH_PARENT高度（需要重新测量）
        boolean matchHeight = false;
        // 是否跳过了某些子View的测量（权重优化）
        boolean skippedMeasure = false;

        // 初始化baseline对齐数组，用于存储不同gravity的最大上升和下降值
        ...
        // 初始化所有gravity方向的ascent和descent为-1
        ...
        // 最大子View宽度，用于useLargestChild模式
        int largestChildWidth = Integer.MIN_VALUE;
        // 已使用的额外空间（用于权重子View的优化）
        int usedExcessSpace = 0;

        // 非跳过的子View数量（非GONE状态的子View）
        int nonSkippedChildCount = 0;

        // 第一次遍历：测量所有子View的宽度和高度
        // 目的：确定非权重子View的尺寸，计算总宽度和最大高度
        for (int i = 0; i < count; ++i) {
            // 获取当前子View
            final View child = getVirtualChildAt(i);
            if (child == null) {
                // 处理null子View（某些子类可能返回null）
                mTotalLength += measureNullChild(i);
                continue;
            }
            ...
            // 检查是否只使用额外空间的权重子View（宽度为0且有权重）
            final boolean useExcessSpace = lp.width == 0 && lp.weight > 0;
            if (widthMode == MeasureSpec.EXACTLY && useExcessSpace) {
                // 优化：当父容器宽度确定且子View只使用额外空间时，跳过测量
                // 这些View将在后续权重分配阶段测量
                if (isExactly) {
                    // EXACTLY模式：直接增加margin
                ...
                // 但是，baseline对齐需要测量View以获取baseline偏移
                // 这会抵消上述优化，让子View使用尽可能多的空间
                if (baselineAligned) {
                ...
                // 需要正常测量的情况
                if (useExcessSpace) {
                    // 宽度模式是UNSPECIFIED或AT_MOST，且子View只使用额外空间
                    // 临时设置为WRAP_CONTENT以获取最优宽度
                    lp.width = LayoutParams.WRAP_CONTENT;
                }

                // 测量子View
                // 如果没有权重，使用已用宽度；如果有权重，允许使用全部可用空间
                ...
                // 如果启用useLargestChild，记录最大子View宽度
                if (useLargestChild) {
                    largestChildWidth = Math.max(childWidth, largestChildWidth);
                }
            }

            // 检查子View是否需要MATCH_PARENT高度（本地匹配）
            boolean matchHeightLocally = false;
            if (heightMode != MeasureSpec.EXACTLY && lp.height == LayoutParams.MATCH_PARENT) {
                // LinearLayout的高度会缩放，且至少有一个子View想匹配我们的高度
                // 设置标志，表示当我们知道自己的高度后需要重新测量该View
                matchHeight = true;
                matchHeightLocally = true;
            }

            // 计算子View的总高度（包含上下margin）
            ...
            if (baselineAligned) {
                final int childBaseline = child.getBaseline();
                if (childBaseline != -1) {
                    // 将子View的垂直gravity转换为索引（0..VERTICAL_GRAVITY_COUNT）
                    // 使用位运算提取gravity信息
                    ...
                    // 更新对应gravity方向的最大ascent和descent
            ...
            // 检查是否所有子View都是MATCH_PARENT高度
            allFillParent = allFillParent && lp.height == LayoutParams.MATCH_PARENT;
            if (lp.weight > 0) {
                /*
                 * 权重子View的高度如果在后续重新测量中可能是无效的
                 * 所以将它们分开存储
                 */
            ...
            // 增加跳过的子View计数（某些布局如TableLayout可能需要）
            i += getChildrenSkipCount(child, i);
        }

        // 如果有非GONE的子View且在最后有分隔线，增加分隔线宽度
        if (nonSkippedChildCount > 0 && hasDividerBeforeChildAt(count)) {
            mTotalLength += mDividerWidth;
        }

        // 处理baseline对齐的高度调整
        // 优先检查INDEX_TOP（Gravity.TOP），这是最常见的情况
        if (maxAscent[INDEX_TOP] != -1 ||
                maxAscent[INDEX_CENTER_VERTICAL] != -1 ||
                maxAscent[INDEX_BOTTOM] != -1 ||
                maxAscent[INDEX_FILL] != -1) {
            // 计算所有gravity方向中的最大ascent和descent
            ...
            // 更新最大高度，确保baseline对齐有足够空间
            maxHeight = Math.max(maxHeight, ascent + descent);
        }

        // 处理useLargestChild模式（在AT_MOST或UNSPECIFIED宽度模式下）
        if (useLargestChild &&
        ...
        // 解析计算出的尺寸与widthMeasureSpec的约束
        int widthSizeAndState = resolveSizeAndState(widthSize, widthMeasureSpec, 0);
        widthSize = widthSizeAndState & MEASURED_SIZE_MASK;

        // 权重分配阶段：扩展权重子View以占用可用空间，或缩小它们以适应边界
        // 如果跳过了某些子View的测量，现在需要测量它们
        int remainingExcess = widthSize - mTotalLength
                + (mAllowInconsistentMeasurement ? 0 : usedExcessSpace);
        if (skippedMeasure
                || ((sRemeasureWeightedChildren || remainingExcess != 0) && totalWeight > 0.0f)) {
            // 计算权重总和：如果设置了weightSum则使用它，否则使用所有子View的权重之和
            float remainingWeightSum = mWeightSum > 0.0f ? mWeightSum : totalWeight;

            // 重置baseline相关数组，准备重新计算
            maxAscent[0] = maxAscent[1] = maxAscent[2] = maxAscent[3] = -1;
            maxDescent[0] = maxDescent[1] = maxDescent[2] = maxDescent[3] = -1;
            maxHeight = -1;
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (childWeight > 0) {
                    ...
                    if (mUseLargestChild && widthMode != MeasureSpec.EXACTLY) {
                        ...
                        // 该子View需要从头开始布局，只使用其分配的额外空间
                        childWidth = share;
                    } else {
                        // 该子View有固有宽度，需要加上分配的额外空间
        ...
        // 如果不是所有子View都是MATCH_PARENT且高度模式非EXACTLY，使用备选最大高度
        ...
        // 设置最终的测量尺寸
        // 宽度：包含子View的状态
        // 高度：解析高度规格并包含子View的垂直状态
        setMeasuredDimension(widthSizeAndState | (childState&MEASURED_STATE_MASK),
                resolveSizeAndState(maxHeight, heightMeasureSpec,
                        (childState<<MEASURED_HEIGHT_STATE_SHIFT)));

        // 如果有子View需要MATCH_PARENT高度，强制统一高度重新测量
        if (matchHeight) {
